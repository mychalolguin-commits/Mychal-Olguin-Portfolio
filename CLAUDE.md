...
Two pairs, because a colour sized for a button is not one you can flood a viewport with: `--brand` / `--on-brand` for interactive elements, `--brand-field` / `--on-brand-field` for the closing field. Light mode is warm paper (`#F7F1E6`) with a deeper pine action (`#174F38`) and warm elevated panels instead of pure white. Dark mode is warm charcoal (`#15120E`) with a brighter action green (`#46B57A`) and a deeper field green (`#164A35`). All important pairs clear 7.2:1. `--surface-sunken` is a low-contrast tint used to alternate section bands so they separate without a rule. Tailwind cannot apply an alpha modifier to an arbitrary `var()`, so translucent values (`--on-brand-muted`) ship as real rgba tokens.
