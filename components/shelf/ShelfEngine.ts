...
/**
 * Colors for the room the shelf sits in. Added so the scene can follow the
 * site's light/dark theme instead of the upstream demo's fixed warm cream.
 * Book colors are unaffected — those come from the catalog.
 */
export type ShelfEnvironment = {
  background: string;
  wall: string;
  ground: string;
...
export const DEFAULT_SHELF_ENVIRONMENT: ShelfEnvironment = {
  background: "#eee8db",
  wall: "#eee8db",
  ground: "#e7dfd0",
...
export class ShelfEngine {
  ...
  private environment: ShelfEnvironment;
  ...
  constructor(
    ...
    environment: ShelfEnvironment = DEFAULT_SHELF_ENVIRONMENT,
  ) {
    this.canvas = canvas;
  ...
  private setupScene() {
    ...
    this.scene.fog = new THREE.Fog(env.background, 10, 26);

    const hemisphere = new THREE.HemisphereLight(
      env.hemisphereSky,
      env.hemisphereGround,
      2.4,
    );
  ...
  /**
   * Recolor the room in place. Lets the site's theme toggle restyle the scene
   * without tearing down and rebuilding the whole shelf.
   */
  setEnvironment(environment: ShelfEnvironment) {
    this.environment = environment;
    if (this.scene.background instanceof THREE.Color) {
      this.scene.background.set(environment.background);
    } else {
      this.scene.background = new THREE.Color(environment.background);
    }
    if (this.scene.fog instanceof THREE.Fog) {
      this.scene.fog.color.set(environment.background);
    }
    this.wallMaterial?.color.set(environment.wall);
    this.groundMaterial?.color.set(environment.ground);
    this.hemisphereLight?.color.set(environment.hemisphereSky);
    this.hemisphereLight?.groundColor.set(environment.hemisphereGround);
  }
  ...
  private handleWheel = (event: WheelEvent) => {
    ...
    // The upstream demo owned the whole viewport, so it could claim every
    // wheel event. Embedded in a scrolling page that would trap the reader:
    // scrolling down the homepage with the pointer over the shelf would stop
    // the page instead. Only horizontal intent (trackpad swipe, shift+wheel)
    // browses the shelf; vertical wheel is left alone to scroll the page.
